...
shadow-flight ticks="1000" seed="7" tick_ms="100" output="data/reports/shadow-flight/latest":
    ulimit -s 32768 && cargo run -q -p pete-tools -- shadow-flight --source fixture --ticks "{{ticks}}" --tick-ms "{{tick_ms}}" --seed "{{seed}}" --clock accelerated --output "{{output}}"
...
vision-eval *args:
    cargo run -q -p pete-tools --no-default-features -- vision-eval {{args}}
social-exam *args:
