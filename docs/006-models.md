...
The online `TinyNowVector` discovery path is tracked in [experience-forge.md](experience-forge.md). Replay and checkpoint artifacts from that forge now feed the first #25 latent round-trip trainer: a small autoencoder learns a compact `Experience` latent from `TinyNowVector` plus action features, decodes range/depth/contact summaries, and trains a future predictor against the next `TinyNowVector`. The report keeps this learned latent distinct from teacher/fallback vectors by comparing it against both the evolved forge vector and a random projection baseline.

The unified #37 trainer is tracked in [unified-experience.md](unified-experience.md). It assembles multimodal teacher-vector slots plus explicit missingness masks into a `UnifiedExperienceInstant`, trains a learned `ExperienceLatent`, and reports decode/predict/compare evidence against copy-current, random projection, mechanical Instant, and Forge baselines. The broader canonical runtime/replay DTO is [ExperienceInstant](instant.md), which replaces the old Sensorium concept for #38.
