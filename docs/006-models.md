...
The unified #37 trainer is tracked in [unified-experience.md](unified-experience.md). It assembles multimodal teacher-vector slots plus explicit missingness masks into a `UnifiedExperienceInstant`, trains a learned `ExperienceLatent`, predicts `ExperienceLatent_t+1`, computes `ExperienceSurprise`, and reports decode/predict/compare evidence against copy-current, random projection, and mechanical Instant research baselines. The canonical durable runtime/replay DTO is [ExperienceInstant](instant.md), which replaces the old Sensorium concept for #38. The canonical current belief state used by policy is [`Now.world`](020-now-world-model.md); the learned latent does not replace it.
