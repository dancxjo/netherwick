...
#31 is considered closed when image, face, voice, and text vectorization are represented in the embodied pipeline while placeholders remain deterministic and available. Netherwick currently registers deterministic feature-backed vectorizers for image, crop, audio, transcript, and depth payloads; preserves real model-backed upstream face and voice `VectorArtifact`s when sensors provide them; and emits deterministic semantic hash vectors for transcript, impression, and experience-summary text. CLIP/OpenCLIP, ArcFace runtime loading, SpeakerNet, LLM embeddings, and other neural model-backed vectorizers remain follow-up work.
