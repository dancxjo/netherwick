...
pub fn register_motherbrain(
    ...
    let generation = REGISTRATION_GENERATION
...
mod tests {
    ...
    fn registration_must_match_live_lease_and_expires() {
        ...
        assert!(register_motherbrain(b"wrong", [192, 168, 4, 2], 5, 1_000).is_none());
        let identity = b"636c69656e742d61";
        assert!(register_motherbrain(identity, [192, 168, 4, 3], 5, 1_000).is_none());
        assert!(register_motherbrain(identity, [192, 168, 4, 2], 5, 1_000).is_some());
        assert_eq!(resolve_motherbrain(5_999), Some([192, 168, 4, 2]));
        assert_eq!(resolve_motherbrain(6_000), None);
    }
}
