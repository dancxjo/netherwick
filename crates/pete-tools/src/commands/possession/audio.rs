fn run_mouth(args: MouthArgs) -> Result<()> {
    let Some(mouth) = QueuedSpeechMouth::from_env()? else {
        anyhow::bail!(
...
fn enqueue_default_bringup_outputs(
    mouth: &Option<QueuedSpeechMouth>,
...
fn speak_robot_mouth_text_before_status(mouth: &QueuedSpeechMouth, text: &str) -> bool {
...
fn enqueue_robot_mouth_text(mouth: &QueuedSpeechMouth, text: &str) {
