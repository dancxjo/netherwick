...
mod tests {
    use super::*;
    use crate::{
        GraphEntity, MemorySense, ObjectClass, ObjectObservation, ObjectObservationSource,
        ObjectSense, VectorArtifact,
    };
    ...
    fn face_voice_mismatch_remains_an_explicit_identity_contradiction() {
        ...
        now.face.vectors.push(
            VectorArtifact::new("faces", "face:alex", vec![0.1]).with_source_id("person:alex"),
        );
        now.voice.vectors.push(
            VectorArtifact::new("voices", "voice:bob", vec![0.2]).with_source_id("speaker:bob"),
        );
        let entity = WorldEntity {
